            ...
            <div style={ownerViewStore}>
                ...
                <div id='inventory' style={ownerViewStore.inventory}>{inventory.map(computer =>
                    <div key={computer.id} style={ownerViewStore.computer}>
                        <div style={ownerViewStore.text}><b>{computer.name}<br />List Price: ${computer.price}</b><br /><br />Memory: {computer.memory}<br />Storage Size: {computer.storage}<br />Processor: {computer.processor}<br />Processor Gen: {computer.processorgen}<br />Graphics: {computer.graphics}<br />Date Added: {(new Date(computer.timecreated)).toUTCString()}</div>
