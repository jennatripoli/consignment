...
def delete(event,context):
    ...
    try:
        ...
        if body['action'] == 'DELETE':
            ...
            [username,manager_balance] = cursor.fetchall()[0]
            cursor.execute(statement,(body['id'],))
            cursor.execute(update_store,(round(inventory-float(body['price']),2),round(balance+float(body['price'])*.95,2),body['store']))
            cursor.execute(update_manager,(round(manager_balance+float(body['price'])*.05,2),username))
            conn.commit()
        else:
